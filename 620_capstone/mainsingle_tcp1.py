...
def send():
    global shared_r_locate, send_location, lidar_signal, arrive, message, f_location, lidar_signal

    server_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM) # AF_INET : 주소 체계, SOCK_STREAM : TCP 방식
    server_address = ('192.168.0.25', 12352) 
    ...
    try :
        ...
        while True :
            ...
            if client_address :
                ...
                with lock :
                    ##################명현################1
                    if arrive and (signal == 'arrive') : # 라이다에서 목표점에 도착했을 때 신호를 받으면 실행
                        msg_arrive = arrive
                    ##################명현################1 
                    
                    ##################혜선################1
                    if prev_send_location != cur_send_location : # mic_speaker로부터 목표지점의 데이터를 전달받았을 때 라이다로 전송
                    ...
                    ##################혜선################2
                    if prev_cur_send_location != cur_cur_send_location : # mic_speaker로부터 현재지점의 데이터를 전달받았을 때 라이다로 전송
                        msg_cur_location = cur_cur_send_location
                        prev_cur_send_location = cur_cur_send_location
                        if msg_cur_location :
                            connection.sendall(msg_location.encode('utf-8'))
                            print(f"라이다에게 현재 {msg_location}의 좌표값을 보냈습니다. ")
